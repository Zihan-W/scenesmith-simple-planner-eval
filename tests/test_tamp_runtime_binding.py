"""A TAMP-selected grasp must reach the existing BT skill runtime unchanged."""
...
class BindingTest(unittest.TestCase):
    ...
    def _check_pick_binding(self, name, registry):
        positions = [0.1] * 7
        checks = {"ik": {
        ...
        with tempfile.TemporaryDirectory() as directory:
            with patch("examples.online_manipulation.tamp_scenesmith_online.make_policy",
                       side_effect=factory):
            ...
            self.assertEqual(captured[0]["tamp_joint_skill_plan"]["approach_waypoints"],
                             [[0.05] * 7, positions])
            tree = json.loads(Path(captured[0]["bt_json_input"]).read_text())
    ...
    def test_missing_or_mismatched_assignment_cannot_claim_conditioning(self):
        ...
        action = ParameterizedSkillAction("NavigateToPick", {"object": "cube"},
            ...
            {"base_pose": "b0"})
        leaf = Node("action", "NavigateTo", tuple(goal))
