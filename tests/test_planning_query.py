...
import numpy as np
from pydrake.all import JointIndex, RigidTransform
...
class PlanningQueryTest(unittest.TestCase):
    ...
    def test_ik_handles_measured_quaternion_norm_drift_without_changing_snapshot(self):
        for scale in (1.0001, 0.9999):
            with self.subTest(scale=scale):
                query = _planning_query()
                query.set_observed_body_poses({
                    "target": Pose((2.0, 2.0, 0.0),
                                   (math.cos(0.2), 0.0, 0.0, math.sin(0.2)))})
                target = query.frame_pose("test_robot", "arm")
                positions = query.plant.GetPositions(query.context).copy()
                fixed = []
                for index in range(query.plant.num_joints()):
                    joint = query.plant.get_joint(JointIndex(index))
                    if joint.num_positions() == 7:
                        start = joint.position_start()
                        positions[start:start + 4] *= scale
                        fixed.extend(range(start, start + 7))
                query.plant.SetPositions(query.context, positions)
                query.synchronize_state(query.context)
                before_pose = query.body_pose("movable", "body")
                result = query.solve_ik(target, seed=query.configuration(),
                                        orientation_tolerance_rad=0.01)
                self.assertTrue(result.success, result)
                self.assertTrue(query.check_configuration(result.configuration).valid)
                np.testing.assert_array_equal(
                    query.plant.GetPositions(query.context)[fixed], positions[fixed])
                after_pose = query.body_pose("movable", "body")
                np.testing.assert_allclose(after_pose.translation_m, before_pose.translation_m)
                np.testing.assert_allclose(after_pose.quaternion_wxyz, before_pose.quaternion_wxyz)
