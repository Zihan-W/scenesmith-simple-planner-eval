...
from examples.online_manipulation.tamp_scenesmith_online import failed_pick_recovery_status, SceneSmithWorldObserver
...
from examples.online_manipulation.tamp_online import IncrementalTampRunner, SkillExecution
...
class RecoveryPreconditionsTest(unittest.TestCase):
    ...
    def test_runner_stops_before_retry_or_new_program(self):
        ...
        runner=IncrementalTampRunner(semantic=Semantic(((holding,),)),registry=registry,solver_factory=lambda world:SamplingSolver(registry,Domain(),batch_size=2),executor=executor,observer=Observer(('red_cube',)),trace=events.append)
