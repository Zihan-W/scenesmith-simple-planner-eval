...
class OnlineRecoveryTest(unittest.TestCase):
    ...
    def test_contact_blocker_is_replanned_but_never_reexecuted(self):
        parked = PredicateGoal("at_pick_pose", ("red_cube",))
        initial = frozenset((self.seen, self.empty, parked))
        after_failure = frozenset((self.seen, parked))
        ...
        class ContactExecutor:
            ...
            def execute(self, action):
                self.calls.append(action.skill_name)
                return SkillExecution(after_failure, False, "joint_edge_rejected",
                                      retryable=False,
                                      failure_details={"recovery_preconditions": {
                                          "allowed": False,
                                          "blockers": ["target_finger_contact",
                                                       "navigation_precheck_failed"]}})

        semantic = Semantic(((self.holding,), (self.holding,)))
        executor = ContactExecutor()
        events = []
        registry = picklift_registry()
        runner = IncrementalTampRunner(
            semantic=semantic, registry=registry,
        ...
        result = runner.run(task="pick red_cube", task_goals=(self.holding,),
                            ...
                            predicate_arity={"holding": 1})
        self.assertFalse(result.success)
        self.assertEqual(executor.calls, ["PickLift"])
        self.assertGreaterEqual(result.metrics["num_skill_replans"], 1)
        self.assertEqual(semantic.calls, 1)
        self.assertIn("target_finger_contact", str(events))
        self.assertTrue(any(e.get("event") == "recovery_blocked" for e in events))

    def test_current_station_pick_checked_before_model_relocation(self):
        parked = PredicateGoal("at_pick_pose", ("red_cube",))
        initial = (self.seen, self.empty, parked)
        registry = picklift_registry()

        class RepositionGenerator:
            calls = 0

            def generate(self, world, goals, **kwargs):
                self.calls += 1
                return SkillProgram((
                    SkillStep("NavigateToPick", {"object": "red_cube"},
                              {"base_pose": "b"}),
                    SkillStep("PickLift", {"object": "red_cube"},
        ...
        self.assertTrue(result.success)
        self.assertEqual(executor.calls, ["PickLift"])
        self.assertTrue(any(e.get("event") == "current_station_pick_check" for e in events))

    def test_failed_current_station_pick_replans_before_deferred_navigation(self):
        parked = PredicateGoal("at_pick_pose", ("red_cube",))
        initial = (self.seen, self.empty, parked)
        failed = (self.seen, parked)
        ...
        class Generator:
            ...
            def generate(self, world, goals, *, feedback=(), **kwargs):
                self.calls += 1
                self.feedback.append(feedback)
                self.worlds.append(world)
                return SkillProgram((
                    SkillStep("NavigateToPick", {"object": "red_cube"}, {"base_pose": "b"}),
                    SkillStep("PickLift", {"object": "red_cube"},
                              {"grasp_pose": "g", "approach_pose": "a"}),
                ))
        ...
        class ContactExecutor:
            ...
            def execute(self, action):
                self.calls.append(action.skill_name)
                return SkillExecution(failed, False, "joint_edge_rejected", retryable=False,
                                      failure_details={"recovery_preconditions": {
                                          "allowed": False,
                                          "blockers": ["target_finger_contact"]}})
        ...
        result = runner.run(task="pick red_cube", task_goals=(self.holding,),
                            initial_observation=initial, initial_geometry_state={},
                            predicate_arity={"holding": 1})
        self.assertEqual(result.reason, "recovery_preconditions_failed")
        self.assertEqual(executor.calls, ["PickLift"])
        self.assertEqual(generator.calls, 2)
        self.assertEqual(result.metrics["num_skill_replans"], 1)
        self.assertEqual(generator.feedback[1][-1]["failure_source"], "execution_failure")
        self.assertNotIn(self.empty, generator.worlds[1].facts)
        self.assertTrue(any(e["event"] == "recovery_candidate_blocked" for e in events))

    def test_relocation_remains_available_after_measured_pick_fails(self):
        parked = PredicateGoal("at_pick_pose", ("red_cube",))
        initial = (self.seen, self.empty, parked)
        registry = picklift_registry()
        ...
        class StationDomain(Domain):
            def check(self, skill, candidate, state):
                if skill.skill == "PickLift" and state.get("base_pose") != 1.0:
                    return False, "ik", {"message": "current station cannot reach"}
                return super().check(skill, candidate, state)

        class MeasuredObserver(Observer):
            def observe(self, facts):
                state = super().observe(facts)
                return WorldState(state.objects, state.facts,
                                  observation_id=str(len(executor.calls)))

            def geometry_state(self, observation, previous):
                return {**previous, "base_pose": 1.0}
        ...
        result = runner.run(task="pick red_cube", task_goals=(self.holding,),
                            ...
                            initial_geometry_state={"base_pose": 0.0},
                            predicate_arity={"holding": 1})
        self.assertTrue(result.success, events[-12:])
        self.assertEqual(executor.calls, ["NavigateToPick", "PickLift"])
        self.assertTrue(any(e.get("event") == "current_station_pick_infeasible"
                            for e in events))
