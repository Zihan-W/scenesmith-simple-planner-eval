...
from simulation.src.geometry.contact import SupportContactPolicy, permits_contact
from simulation.src.geometry.scene_geometry import resolve_ground_geometries
from simulation.src.scene.scene_input import load_dmd
...
class StructureMigrationTest(unittest.TestCase):
    ...
    def test_formal_modules_do_not_depend_on_examples_or_monitor(self):
        paths = list((ROOT / "simulation/src").rglob("*.py"))
        for path in paths:
            for node in ast.walk(ast.parse(path.read_text())):
                if isinstance(node, ast.ImportFrom):
                    self.assertFalse((node.module or "").startswith(("planner", "scripts", "tools")), path)
        self.assertNotIn("zerith_grasp_geometry", (ROOT / "simulation/src/robots/adapters/zerith.py").read_text())

    def test_external_policy_evaluator_reset_and_artifacts(self):
        with tempfile.TemporaryDirectory() as folder:
            outside = Path(folder)
            shutil.copyfile(ROOT / "simulation/examples/external_evaluator.py", outside / "my_components.py")
            ...
            result = subprocess.run([
                sys.executable, "-B", "-m", "simulation.src", str(file),
    ...
    def test_control_settings_do_not_change_hardware_limits(self):
        ...
        from simulation.src import JointSpec
