...
from src.online_manipulation import BaseVelocityAction, JointDeltaAction, Pose, RobotCommand, make_env
...
class MeasuredGeometryTest(unittest.TestCase):
    ...
    def test_query_uses_measured_joints_and_reuses_only_the_same_base(self):
        ...
        domain = object.__new__(SceneSmithPickDomain)
        domain.config, domain.observation = config, observation
        ...
        self.assertIs(domain._candidate_query(base)[0], query)
        ...
        self.assertIs(restored_query, nav_query)
        restored_base = nav_query.frame_pose(adapter.spec.model_instance_name,
                                             adapter.spec.base_link_name)
