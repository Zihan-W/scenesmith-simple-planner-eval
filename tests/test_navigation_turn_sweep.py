...
class NavigationTurnSweepTest(unittest.TestCase):
    def check_goal(self, x, y, yaw):
        ...
        def fork_query():
            copied_root = root.Clone()
            return SimpleNamespace(plant=plant,
                context=plant.GetMyMutableContextFromRoot(copied_root),
                robot_model_instance=robot, root_context=copied_root)
        query.fork = fork_query
