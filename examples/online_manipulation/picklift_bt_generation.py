"""Versioned scene-and-task-to-BT generation module for SceneSmith PickLift.
...
RESULT_SCHEMA = "scenesmith.picklift_bt_generation.result.v1"
PROMPT_VERSION = "microsoft-scene-aware-picklift-first-person-v1"
...
class ChatClient(Protocol):
    ...
    def complete(self, *, model: str, messages: list[dict]) -> ChatCompletion:
        ...
...
def load_request(request_path) -> LoadedRequest:
    """Load and strictly validate the fixed v1 request contract."""
    ...
    validate_inputs(environment, task_plan)

    observations = document["observations"]
    if not isinstance(observations, list) or len(observations) != len(CAMERA_ORDER):
        raise ValueError("observations must contain head and left-wrist images")
    images = []
    for index, (observation, expected_camera) in enumerate(
        zip(observations, CAMERA_ORDER, strict=True), 1
    ):
        _expect_object(
...
def _skill_prompt() -> str:
    definitions = {
        ...
        "ExecutePickLift": {
            "class": "action",
            "arguments": [],
            "description": "Run the calibrated SceneSmith PickLift controller until the observed target is lifted and stably held, or the controller fails.",
        },
        "PickLiftSucceeded": {
            "class": "condition",
            "arguments": [],
...
def build_messages(loaded: LoadedRequest) -> list[dict]:
    ...
    contract = (
        ...
        "prose. All arguments are double-quoted JSON strings. Wait takes one decimal argument; "
        "ExecutePickLift takes no arguments. ULTIMATE_GOAL must be a non-empty natural-language "
...
def _repair_prompt(error: Exception) -> str:
    return (
        ...
        "sequence string. Preserve task steps once and in order. ExecutePickLift takes no "
        "arguments. Do not emit Markdown or prose."
    )


def generate(loaded: LoadedRequest, client: ChatClient) -> dict:
    """Generate, strictly compile, and return the fixed v1 result document."""
    ...
    for number in range(1, model["max_attempts"] + 1):
        ...
        try:
            response, root = compile_response(
...
def write_result(result: dict, output_dir) -> Path:
    ...
    # The result tree is already validated; reconstruct the root only for Mermaid through
...
def _task_from_result(result: dict) -> dict:
    """Recover the finite task-plan shape solely for rendering a validated result."""

    sequence = result["tree"]["children"][0]["children"][1]
    steps = []
    for node in sequence["children"]:
        if node["name"] == "Wait":
            steps.append({"skill": "Wait", "duration_s": float(node["args"][0])})
        else:
            steps.append({"skill": node["name"]})
    return {"success_condition": "PickLiftSucceeded", "steps": steps}
...
def main(argv=None):
    parser = argparse.ArgumentParser(
        description="Generate a strictly compiled PickLift BT from one v1 request document."
    )
