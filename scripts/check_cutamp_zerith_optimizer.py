...
from build_zerith_curobo_collision import distance_mesh
from cutamp_stationary_contacts import StationaryTargetWorldCost
from cutamp_stationary_support import StationarySupportWorldCost
...
def seed_calibrated_grasps(initializer, calibration, target_spec, body_from_center,
    ...
    offsets = torch.rand(count, device=world.tensor_args.device) * 0.04 - 0.02
...
def exported_base_bounds(document, yaw):
    """Validate and return the existing world-axis-aligned base domain."""
    candidates = np.asarray(document["base_candidates_xyz_yaw"])
    if not np.allclose(np.cos(candidates[:, 2]), np.cos(yaw), atol=1e-7) or not np.allclose(
            np.sin(candidates[:, 2]), np.sin(yaw), atol=1e-7):
        raise ValueError("The exported base domain heading differs from the optimization anchor")
    bounds = np.stack((candidates[:, :2].min(axis=0), candidates[:, :2].max(axis=0)))
    for x in bounds[:, 0]:
        for y in bounds[:, 1]:
            if not np.any(np.all(np.isclose(candidates[:, :2], [x, y], atol=1e-8, rtol=0), axis=1)):
                raise ValueError("Rotated base domain requires its actual convex constraints, not an AABB")
    return bounds
...
def main():
    ...
    for record in document["geometries"]:
        ...
        if record["model"] == target_spec["model_instance_name"] and record["body"] == target_spec["body_name"]:
            ...
            matrix = base_from_world @ np.array(record["world_from_geometry"]) @ box.primitive.transform
            statics.append(Cuboid(name=f"drake_{record['id']}", pose=pose_list(matrix),
                                   dims=box.primitive.extents.tolist()))
    ...
    if base_bounds is not None:
        bounds_tensor = tensor_args.to_device(base_bounds)
        base_samples = (bounds_tensor[0] + torch.rand((args.num_particles, 2), device="cuda")
    ...
    reducer = CostReducer(json.loads(args.multipliers.read_text()))
    checker = ConstraintChecker(json.loads(args.tolerances.read_text()))
