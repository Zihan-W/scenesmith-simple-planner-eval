...
from src.item_locking_monitor import _CalcAabb, _OabbToAabb
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=float,
        help=(
            "Calibrated world yaw; defaults to the source target yaw while "
            "removing its residual roll and pitch."
        ),
    )
...
def _calibrated_pose(
    *,
    ...
    """Compute a centered, level box pose on the table collision top."""
    ...
    target_xyz_world = X_WT_source.translation().copy()
    target_xyz_world[2] = table_max[2] + 0.5 * BOX_SIZE_METERS[2]
    X_WT_calibrated = RigidTransform(
        RollPitchYaw(0.0, 0.0, yaw_rad),
        target_xyz_world,
    )

    base_frame_name = _target_base_frame(source_text, target_model_name)
    ...
    X_BT_calibrated = X_WB.inverse().multiply(X_WT_calibrated)
    relative_rpy_deg = np.rad2deg(
    ...
    calibration = {
        ...
        "source_target_body_xyz_world_m": (
            X_WT_source.translation().tolist()
        ),
        ...
        "calibrated_base_frame": base_frame_name,
        "calibrated_translation_in_base_frame_m": (
            X_BT_calibrated.translation().tolist()
        ),
        "calibrated_rpy_in_base_frame_deg": relative_rpy_deg.tolist(),
        "xy_position_source_preserved": True,
        "roll_pitch_leveled": True,
    }
    return (
...
def _refine_support_pose(
    *,
    ...
    target_model_name: str,
    table_model_name: str,
    target_base_frame_name: str,
) -> tuple[np.ndarray, np.ndarray, dict]:
    """Refine box Z until its signed distance to the table is zero."""
    _, plant, plant_context = _build_scene(
        derived_dmd,
    ...
    base_frame = plant.GetFrameByName(target_base_frame_name)
    X_WB = plant.CalcRelativeTransform(
        plant_context,
        plant.world_frame(),
        base_frame,
    ...
    def distance_at_z(z_world: float) -> float:
        X_WT_test = RigidTransform(
            X_WT.rotation(),
            np.r_[X_WT.translation()[:2], z_world],
        )
        plant.SetFreeBodyPose(
            plant_context,
            target_body,
            X_WB.inverse().multiply(X_WT_test),
        )
    ...
    upper_z = float(X_WT.translation()[2])
    upper_distance = distance_at_z(upper_z)
    if upper_distance < 0.0:
        raise ValueError(
            "Analytic box support pose unexpectedly penetrates the table"
        )
    lower_z = upper_z - BOX_SIZE_METERS[2]
    lower_distance = distance_at_z(lower_z)
    if lower_distance > 0.0:
        raise ValueError(
            "Could not bracket the local coffee-table support surface"
        )
    initial_upper_distance = upper_distance
    for _ in range(60):
    ...
    final_xyz_world = X_WT.translation().copy()
    final_xyz_world[2] = upper_z
    X_WT_refined = RigidTransform(X_WT.rotation(), final_xyz_world)

    X_BT_refined = X_WB.inverse().multiply(X_WT_refined)
    rpy_deg = np.rad2deg(
        RollPitchYaw(X_BT_refined.rotation()).vector()
    )
    return (
        X_BT_refined.translation(),
        rpy_deg,
        {
            "analytic_candidate_center_z_world_m": float(
                X_WT.translation()[2]
            ),
            "analytic_candidate_table_distance_m": initial_upper_distance,
            "refined_center_z_world_m": upper_z,
            "refined_local_table_top_z_world_m": (
                upper_z - 0.5 * BOX_SIZE_METERS[2]
            ),
            "refined_target_table_signed_distance_m": upper_distance,
            "penetrating_bracket_distance_m": lower_distance,
            "method": (
                "vertical signed-distance bisection at fixed XY and yaw"
            ),
        },
    )
...
def _task_metadata(
    ...
    return {
        ...
        "locked_rail_pick_results_status": (
            "wip_only_not_approved_for_approach_or_grasp"
        ),
        "grasp": {
...
def main() -> None:
    ...
    translation, rpy_deg, calibration = _calibrated_pose(
    ...
    translation, rpy_deg, support_refinement = _refine_support_pose(
        derived_dmd=output_dmd,
        scene_package_xml=scene_package_xml,
        target_model_name=args.target_model_name,
        table_model_name=args.table_model_name,
        target_base_frame_name=target_base_frame_name,
    )
    derived_text = _replace_model_pose(
        derived_text,
        args.target_model_name,
    ...
    output_dmd.write_text(derived_text, encoding="utf-8")
    calibration["support_surface_refinement"] = support_refinement
