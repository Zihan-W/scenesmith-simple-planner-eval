...
from simulation.src.runtime.assembly import load_factory, run
from simulation.src.tasks.evaluation import EvaluatedTask, Evaluator, TaskResultEvaluator
from simulation.src.core.protocols import Policy, RobotAdapter, Task
from simulation.src.runtime.runtime import RuntimeConfig
from simulation.src.control.controller import configure_joint_servos
from simulation.src.scene.scene_input import load_dmd, prepare_scene, inspect_dependencies, package_roots
from simulation.src.core.specs import ObservedBodySpec, ScenarioSpec, TimingConfig, VisualizationConfig
from simulation.src.tasks.tasks import NullTask, PickLiftTask, PickLiftTaskConfig
...
def load_experiment(config_path, *, repository_root, cache_root, scene_root=None,
    ...
    if "factory" in robot:
        ...
        from simulation.src.robots.adapters.zerith_dual import make_zerith_dual_spec, ZerithDualRobotAdapter
    ...
    if "factory" in p:
        ...
        from simulation.src.control.policies import HoldPolicy
        policy = HoldPolicy()
    ...
    resolved = {"user_config": user, "selected_profiles": selected, "robot_spec": _json(spec),
                ...
                "provenance": {"experiment": str(path), "experiment_sha256": hashlib.sha256(path.read_bytes()).hexdigest(),
                               ...
                               "servo_defaults": "simulation/src/robots/zerith_servo_config.py (validated simulation, not hardware specification)",
